...
#colnames(highqtime) <- c("Category", Question", "Time in Milliseconds")
#colnames(lowqtime) <- c("Category", Question", "Time in Milliseconds")
...
Actually, the above is lying to you. The one that people took longest on was EXT_1 (The first question on the quiz, with an average score of 879626.63), likely because of loading or because they started the question then got busy doing something else. So, I've removed it from the table.
...
# rtimedf <- data.frame(rtime)
# 
# rtimedf
# 
# ggplot(rtimedf,aes(x=factor(0), rtime))+geom_boxplot()
# 
# rtime
# 
# box <- data.frame (question = c("EXT1", "EXT2"), 
#                    values = rtime$EXT1_E, rtime$EXT2_E)
# 
# ggplot(rtime,aes(x=factor(0),"EXT_1_E"))+geom_boxplot()
...
- show legend if you hover...? for the top 25 pie chart 
- statistical distrbution of scores (or at least, agreeable, openness)
