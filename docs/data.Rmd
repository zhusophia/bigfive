...
We can also look at the 'average' scores of each personality trait.^[The average values will actually shift every time you refresh this webpage, as I found a function to randomly grab 100 people, then take the average!] 
...
This data from 224 unique locations/contains 224 different ISO country codes.^['Country codes' are kinda misleading. There are ~195 countries, and ISO has 249 different codes. This is because the ISO contains subdivisions of countries, e.g. Caymen Islands (UK), Christmas Island (Australia), and the Holy See.] Let's dig through this data - a fun bit of stalking!

```{r, country data}


countrydata <- read.csv("country.csv")

countres <- countrydata %>% 
  select('country') %>%
  group_by(country) %>%
  summarize(total = n()) %>%
  arrange(desc(total)) %>% 
...
```{r}
coutdata <- cbind(finaltable$finalletters, countrydata$country) 
...
As for a note on the data: Locations with less than 5 responses have been omitted from the data, as they often significantly skewed maps. 


```{r extscores world map, layout="l-body-outset", fig.cap="Diamond sadasds"}

exttablec <- exttable %>% 
  cbind(countrydata$country)
...
exttablec<- subset(exttablec, region!="NE")
exttablec <- subset(exttablec, region!="TD")
exttablec <- subset(exttablec, region!="PW")
exttablec <- subset(exttablec, region!="KM")
exttablec <- subset(exttablec, region!="TJ")
exttablec <- subset(exttablec, region!="GQ")
exttablec <- subset(exttablec, region!="ML")
...
knitr::kables(list(
  kable(caption = "Most Extroverted", topext) %>% kable_styling(),
  kable(caption = "Most Introverted", botext) %>% kable_styling()
  )
) %>% kable_styling()
...
When looking at extrovertism, North and South America are generally more introverted, often with negative averages, while Asian countries are more extroverted. Europe seems to be the most introverted, with Africa having a diverse range of scores. 

This is the opposite of stereotypes, as Asians are often seen to value 'education' - a largely individual pursuit. On the other hand, Americans seem to be known as 'loud' and 'boisterous', as culture almost demands "friendliness" as a form of being polite, which can be confused for extroversion.  A good lesson in sterotypes! 

Africa seems to have the most diverse range, with the bright blue representing the Democratic Republic of the Congo (CD) and the dark blue of SD representing Sudan. 
...
esttablec<- subset(esttablec, region!="NE")
esttablec <- subset(esttablec, region!="TD")
esttablec <- subset(esttablec, region!="PW")
esttablec <- subset(esttablec, region!="KM")
esttablec <- subset(esttablec, region!="TJ")
esttablec <- subset(esttablec, region!="GQ")
esttablec <- subset(esttablec, region!="ML")
...
knitr::kables(list(
  kable(caption = "Most Calm", topest) %>% kable_styling(),
  kable(caption = "Most Limbic", botest) %>% kable_styling()
  )
) %>% kable_styling()
...
csntablec<- subset(csntablec, region!="NE")
csntablec <- subset(csntablec, region!="TD")
csntablec <- subset(csntablec, region!="PW")
csntablec <- subset(csntablec, region!="KM")
csntablec <- subset(csntablec, region!="TJ")
csntablec <- subset(csntablec, region!="GQ")
csntablec <- subset(csntablec, region!="ML")
...
knitr::kables(list(
  kable(caption = "Most Contentiousness", topcsn) %>% kable_styling(),
  kable(caption = "Least Contentiousness", botcsn) %>% kable_styling()
  )
) %>% kable_styling()
...
agrtablec<- subset(agrtablec, region!="NE")
agrtablec <- subset(agrtablec, region!="TD")
agrtablec <- subset(agrtablec, region!="PW")
agrtablec <- subset(agrtablec, region!="KM")
agrtablec <- subset(agrtablec, region!="TJ")
agrtablec <- subset(agrtablec, region!="GQ")
agrtablec <- subset(agrtablec, region!="ML")
...
knitr::kables(list(
  ...
  kable(caption = "Most Egocentric", topagr) %>% kable_styling()
  )
) %>% kable_styling()
...
opntablec<- subset(opntablec, region!="NE")
opntablec <- subset(opntablec, region!="TD")
opntablec <- subset(opntablec, region!="PW")
opntablec <- subset(opntablec, region!="KM")
opntablec <- subset(opntablec, region!="TJ")
opntablec <- subset(opntablec, region!="GQ")
opntablec <- subset(opntablec, region!="ML")
...
knitr::kables(list(
  ...
  kable(caption = "Least Inquisitve", topopno) %>% kable_styling()
  )
) %>% kable_styling()

```

Higher scores are correlated with inquisition, lower scores are correlated non-inquisition.  
