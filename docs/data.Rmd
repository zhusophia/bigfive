...
A note of the data: I've removed values above 20 seconds^[as they're significant outliars, and make the data difficult to visualize] in the graph and only took a random sample of 50000 response times for each question^[50000 is enough to show the general trend, and utilizing more only makes graphs take longer to load.]. 
...
```
I've taken the liberty of removing EXT_1 (the first question on the quiz), which had an average score of 87 seconds. I'd assume it's skewed because people might've wanted to scroll around the webpage and get accustomed to it or maybe started then quiz then forgot about it. 
