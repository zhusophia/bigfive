...
See, if Open Psychometrics recorded the respondents' results, it would be quite quick to answer this question. Unfortunately, they didn't, and we're working with a bunch of numbers! This is what the data looks like: 
...
We can start by calculating the scores for the 'Extroversion' personality trait. We can do this by summing up the scores of the 10 EXT (standing for EXTroversion) questions. Of course, it should be noted that answering a '4' or '5' on a EXT question doesn't always mean that a user is more extroverted. For example, answering a 5 on EXT Question 2: I don't talk a lot vs EXT Question 1: I am the life of the party would significantly change the results. 

So, we'll need to make sure to add and subtract scores as necessary - I've arbitrarily made the decision to add 'points' if a question symbolizes extroversion, while subtracting 'points' if a question is correlated with introversion. I've made the key below to help with your understanding: 


```{r, exttablekey}
extqs = c("I am the life of the party.", "I don't talk a lot.")
extkey = c("+", "-")
...
Hence, after adding up all their points from each question, any positive total score (> 0) will mean that the user receives an 'S' for Sociable, while a negative score (< 0) would result in an 'R'. If their score is 0, then they'd receive an 'X', as their results since they're perfectly in-between, and will remain inconclusive.
...
The same processes will be applied to the other personality traits^[The neuroticism table was actually calculated differently. For the other 4 traits, there were an equal number of positively-keyed questions (i.e. relating to extroversion) and negatively-keyed questions (i.e. relating to introversion). Neuroticism, on the other hand, had 2 'positive' questions and 8 'negative' questions. So, I subtracted 3 from each result, where a score of 5 (Agree) became +2, and a score of 1 (negative) became -2. This ensured that the results weren't skewed by the amount of negatively-keyed questions. I tried comparing this method with the aforementioned method used to calculate extroversion results, and ended up getting the same results. So, am glad to see that they both work!], with their graphs located below: 

<!-- just explain the process without the footnote, because that what we ended up having to do anwyays.-->
...
```{r, est}
 ...
 pest <- ggplot(data=dfest, aes(x=estlettermap, y=estlettercount)) +
   geom_bar(stat="identity", fill="steel blue") +
   labs(
     title = "Respondents are More Prone to Being Limbic Than Calm",
...
```{r, csn}
 ...
 pcsn <- ggplot(data=dfcsn, aes(x=csnlettermap, y=csnlettercount)) +
   geom_bar(stat="identity", fill="steel blue") +
   labs(
     title = "Responders Tend to be Organized, Rather Than Unorganized",
...
dfagr <- data.frame(agrlettercount = c(846333, 140674, 26551),
 ...
 pagr <- ggplot(data=dfagr, aes(x=agrlettermap, y=agrlettercount)) +
   geom_bar(stat="identity", fill="steel blue") +
   labs(
     title = "Respondents Rate Themseleves More Agreeable Than Egocentric",
...
We can also look at the 'average' scores of each personality trait.^[The average values will actually shift every time I update the website (on RStudio!), as I found a function to randomly grab 100 people then take the average, with the average changing every time I create a version of this webpage!] 
...
colnames(avgscore) <- c("EXT", "EST", "CSN", "AGR", "OPN")
# be careful of slice sample since it randomizes every time
kable(avgscore) %>% 
  kable_styling()
