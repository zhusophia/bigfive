...
Organized, Rather Than Unorganized",
 ...
 ### maybe you can make the graph itself smalelr?
...
Themselves as Inquisitive",
     subtitle = "BREAKING NEWS: People like learning, \n schools are just ineffective... allegedly\n") + 
   xlab("Possible Letters") +
   ylab("Amount of Responses")+
...
<!-- can we zoom out on the graphs?? 
https://gis.stackexchange.com/questions/154050/is-it-possible-to-zoom-out-a-plot-of-a-map-in-r
https://www.tutorialspoint.com/how-to-create-a-plot-in-r-with-a-different-plot-window-size-using-plot-function  
https://stackoverflow.com/questions/51050373/decrease-the-size-of-a-graph-in-ggplot-r
-->

```{r, combineresults, layout="l-body-outset"}
...
Honestly, I’m not too sure why 15% of people are SCOAI, maybe people are being influenced to answer what they WANT to be like, rather than what they actually are, since SCOAI seems like one of the most 'socially-successful’ results.^[Extroverts, stable, organized, agreeable, and inquisitive are all attributes that allow people to thrive, as they will tend to have larger social circles, better intrapersonal relationships and private lives, and the ability to study and work hard to get to positions they want academically and corporately.] So, in reality, I think that many respondents are not actually SCOAIs. That’s my best guess!^[If you have more ideas, I’d love to hear them! Reach out :)]
...
At first, I was surprised that 'XXXXX' didn't appear, since I'd assume it is(theoretically) the most unlikely.^[There's actually 3794 results for XXXXX in this data set] However, I wouldn't be surprised if only 5% of respondents were geniunely XXXXX, while the other 95% of people who got the result just kept clicking 3 (Neutral) for every question, or had some kind of game to see if they could get the (theoretically) super rare XXXXX. 

### Do results vary between countries? <a name="country"></a>

This data from 224 unique locations/contains 224 different ISO country codes.^['Country codes' are kinda misleading. There are ~195 countries, and ISO has 249 different codes. This is because the ISO contains subdivisions of countries, e.g. Caymen Islands (UK) and Christmas Island (Australia)] Let's dig through this data - a fun bit of stalking!
...
ggplot(data= countres, mapping = aes(x = reorder(country, -total), y = total))+
  ...
  theme_classic()+
  labs(title = "Countries of Responders", 
...
Google's SEO (Search Engine Optimization) is also affected by location, and can rank websites by their proximity to the user^[https://www.searchenginejournal.com/ranking-factors/physical-proximity-to-searcher]. So, it's possible that Open Psychometrics is American^[I couldn't find any location data on the website itself], and when Americans search up "Big Five Personality Test", this is the first quiz that shows up.^[As a Canadian, it's actually the third search result!]
