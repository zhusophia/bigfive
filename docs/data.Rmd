...
output: 
  html_document: 
    includes: 
      in_header: loadinganimation.html
    fig_caption: TRUE
date: "2023-08-04"
---
...
#can you remove this from the entire dataset, after thsi point
...
for (i in redcount) {
  exttablec<- filter(exttablec, region!=i)
}
...
- East Asia is slightly more extroverted than introverted
...
for (i in redcount) {
  esttablec<- filter(esttablec, region!=i)
}
...
for (i in redcount) {
  csntablec<- filter(csntablec, region!=i)
}
...
for (i in redcount) {
  agrtablec<- filter(agrtablec, region!=i)
}
...
for (i in redcount) {
  opntablec<- filter(opntablec, region!=i)
}
...
"I shirk my duties" took the longest, likely because 'shirk' is an uncommon word and people may have searched up thed efintion - increasing the amount of time it took to answer. In addition, questions that look longer often had more words to read and often focused on situation prompts. For example, people are rarely "always relaxed" or "never relaxed", so it takes time to think whether you're relaxed a MAJOURITY of the time. Same thing for little concern for others - there are tons of people you care about, but also, tons of people you don't. So, do you take everybody into consideration? Do you focus more on the people close to you to answer this question? it's a bit tricky to have an answer at the top of your head for these cases.  

On the other hand, the questions that took the least amount of time are short and straightforward, and are obvious to judge yourself on. You either talk pretentiously to seem smart, or you don't. I do find it interesting that it's difficult for people to determine whether they're 'relaxed' rather than 'blue'. Likely because you consciously pay attention to when you're sad, but not when you're relaxed? You can also see that these questions are the ones that are asked later (i.e. AGR_9 is part of the 9th out of 10 rounds of questions, more specifically, the 44th question they've read). Anticipation for results may have caused them to rush through the later questions. 
