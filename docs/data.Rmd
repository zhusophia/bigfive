...
est9g <- ggplot(data=est9p, aes(x=Answers)) +
   ...
   labs(
     ...
     subtitle = "THINK GOOD! STARTUP IDEA STAT!") + 
   xlab("Respondent's Possible Answers") +
   ylab("Amount of Responses")+
   theme_minimal()
...
#  summarize(mean = mean(extscore))

#exttablec$region <- countrycode(exttablec$region, "iso2c", "iso3c")

exttablec <- exttablec %>%
  group_by(region) %>%
  arrange(region) %>%
  drop_na(region) %>% 
  slice(-(118: 6754)) %>% #mean can't calculate values of like 1000
  slice(-(702:44700)) %>%
  slice(-(1889:1900))
...
#remoev canada then test out the map again -> don't want this not working in the end
...
#CA, CH, CL, CO, CR, CU, CY, cz, DE, DK, DO, EC, ES, FI, FJ, FR, GB, GR, GT, HK, HN, HR, IE, IL, IN, IT, JE, KH, LK, LU, MX, MY, NI, NL, NO, NZ, PE, PH, PK, PL, PR, PT, RO, RU, SA, SE, SG, SR, SV, TH, TR, TT, TW, US, UY, VE, VN, ZA, ZM
...
mergemap <- inner_join(world_map, extworld, by = "region")


#mergemap
### drop rows that have na in column r to drop, if mean is N/A

ggplot() + 
  geom_polygon( data=mergemap, 
          aes(x=long, y=lat, group=group, fill = mean), 
          color="white", size = 0.2) 

## why is south america and north america gone?? does russia not exist??
...
- mapping averages of each country onto map (https://www.kaggle.com/code/psqrtpsqrt/big-five-personality-test-eda)
-- https://www.youtube.com/watch?v=AgWgPSZ7Gp0&ab_channel=StatisticsGuideswithDrPaulChristiansen
- either refer to responders or users, or something consistent 
-->